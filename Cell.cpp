...
namespace Netlist
{
	...
	Cell* Cell::fromXml( xmlTextReaderPtr reader )
	{
		...
		while( true )
		{
			...
			switch ( state )
			{
				...
				case BeginTerms:
				  ...
				  {
					state = EndTerms;
					// std::cout << "Cell::fromXml() : State : End Terms" << std::endl;
					continue;
				  }
				  break;
				...
				case EndTerms:
					if( (nodeName == termsTag) and (xmlTextReaderNodeType(reader) == XML_READER_TYPE_END_ELEMENT) )
					{
						...
						// std::cout << "Cell::fromXml() : State : Begin Instances" << std::endl;
						continue;
					}
					else
					{
				...
				case BeginInstances:
					...
					{
						state = EndInstances;
						// std::cout << "Cell::fromXml() : State : End Instances" << std::endl;
						continue;
					}
					break;
				...
				case EndInstances:
					...
					{
						state = BeginNets;
						// std::cout << "Cell::fromXml() : State : Begin Nets" << std::endl;
						continue;
					} 
					else
					{
				...
				case BeginNets:
					...
					{
						state = EndNets;
						// std::cout << "Cell::fromXml() : State : End Nets" << std::endl;
						continue;
					}
				...
				case EndNets:
					if( (nodeName == netsTag) and (xmlTextReaderNodeType(reader) == XML_READER_TYPE_END_ELEMENT) )
					{
						...
						// std::cout << "Cell::fromXml() : State : Begin Symbol" << std::endl;
						continue;
					}
					else
					{
				...
				case BeginSymbol:  // TME7
					if( (nodeName == symbolTag) and (xmlTextReaderNodeType(reader) == XML_READER_TYPE_ELEMENT) )
					{
						...
						{
							state = EndCell;
							// std::cout << "Cell::fromXml() : State : End Cell" << std::endl;
							continue;
						}
					}
					break;
		...
		std::cout << "The cell was loaded successfully" << std::endl;
